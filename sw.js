...
const CACHE_VERSION = 'v1.1.0'; // TEP kill-switch: change this string to drop every previous TEP cache
...
const PRECACHE_URLS = [ // Small static set; never list HTML or POST endpoints here
  '/manifest.json', // TEP PWA identity file
  '/favicon.ico', // Existing site favicon
  '/img/e.png', // Existing TEP mark used by the manifest
...
self.addEventListener('fetch', function (event) { // Route GET traffic: API Network-First, static Cache-First
  ...
  if (/\.php$/i.test(url.pathname)) { // Session HTML (index.php, login.php) stays network-only
    return; // Preserves AngularJS first paint and PHP sessions from live Apache
  }
  if (!isStaticAsset(url)) { // HTML navigations and unknown types stay network-only
